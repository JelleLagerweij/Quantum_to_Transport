...
class PP_OCTP:
    ...
    def mandatory_properties(self):
        ...
        # Storing the results in dataframe.
        T_ave, T_sig = np.mean(T), np.std(T)
    ...
    def pressure(self, plotting=False, mov_ave: Union[bool, int]=False):
        """
        ...
        p_ave, p_sig = np.mean(p_l), np.std(p_l)
    ...
    def tot_energy(self, plotting=False, mov_ave=False, unit_conversion=4.184):
        ...
        # Computing total mean and handling uncertainties correctly.
        E_ave, E_sig = np.mean(E_l), np.std(E_l)
    ...
    def pot_energy(self, plotting=False, mov_ave=False, unit_conversion=4.184):
        ...
        # Computing total mean and handling uncertainties correctly.
        E_ave, E_sig = np.mean(E_l), np.std(E_l)
        self.results['Potential Energy/[kJ]'] = [E_ave, E_sig, len(self.f_file)]

    def density(self, u_mass_box):
        """
        ...
        Parameters
        ----------
        u_mass_box : total molar mass in u = sum_tyes(N_atom*u_atom)).
        ...
        for i in range(len(self.f_file)):
            m = u_mass_box/(1000*co.N_A)  # from u to kg
            V = self.V[i]
            rho[i] = m/V
