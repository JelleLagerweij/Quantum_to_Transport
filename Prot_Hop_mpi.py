...
class Prot_Hop:
    ...
    def __init__(self, 
        ...
        self.error_flag = 0
        self.cheap: bool = cheap
        self.folder = os.path.normpath(folder)
        self.verbose: bool = verbose
        self.xyz_out: bool | str = xyz_out
        self.serial_check: bool = serial_check
        self.r_max_count: float = r_hb_old
        self.center: bool = center  # has to be a boolean
        self.store_force_test: bool = store_force_test
        ...
        for i in range(len(subsimulations)):
            ...
            if i == 0:
                try:
                    ...
                    skips = 1
                self.pos_all: np.ndarray = self.df['intermediate/ion_dynamics/position_ions'][()]
                ...
                self.L: float = self.df['results/positions/lattice_vectors'][()][0, 0]  # Boxsize
                self.pos_all *= self.L
                ...
                self.pos_all: np.ndarray = np.concatenate((self.pos_all, pos), axis=0)
    ...
    def setting_properties_all(self):
        ...
        self.L: float = self.comm.bcast(self.L, root=0)
    ...
    def save_results_all(self) -> None:
        ...
        if self.xyz_out is not False:
            ...
            if self.rank != 0:
                self.pos_all: np.ndarray = np.empty(shape, dtype=np.float64)
                self.pos_pro: np.ndarray = np.empty(shape, dtype=np.float64)

            self.comm.Bcast(self.pos_pro, root=0)
            self.comm.Bcast(self.pos_all, root=0)

            if self.rank == 0:
                if self.store_force_test:
                    self.write_force_test(50)
    ...
    def write_to_xyz_all(self, type, center=False) -> None:
        ...
        """
        Additional attempt to store output for two specific snapshots
        """
        if type == 1:
            configs = [None]*2
            j = 0
            for i in range(self.pos_all.shape[0]):
                if (i == 29936) | (i == 114397):
                    configs[j] = Atoms(types, positions=pos[i, :, :], cell=[self.L, self.L, self.L], pbc=True)
                    j += 1
            io.write(os.path.normpath(self.folder+"/2snapshots"+file_ext), configs, format=format, parallel=False)

    def write_force_test(self, n_files: int=50) -> None:
        """
        Generate and save a test set of atomic configurations.
        This method generates a specified number of atomic configuration files
        from the simulation data and saves them in VASP POSCAR format. The files
        are saved in a directory named 'test_set_normal' within the specified folder.
        Parameters
        ----------
        n_files : int, optional
            The number of configuration files to generate (default is 50).
        Returns
        -------
        None
        """
        spacing = int(self.pos_all.shape[0] / n_files)

        ts = np.arange(0, n_files, dtype=int) * spacing
        types = ['H']*self.N_H + ['O']*self.N_O + ['K']*self.N_K
        
        save_path = os.path.normpath(self.folder + "/test_set_normal/")
        if os.path.exists(save_path):
            os.system(f'rm -rf {save_path}')
        os.makedirs(save_path, exist_ok=False)

        for i, t in enumerate(ts):
            state = Atoms(types, self.pos_all[t, :, :], cell=[self.L, self.L, self.L], pbc=True)
            io.write(os.path.normpath(save_path + f"/POSCAR_"+str(i+1)), state, format='vasp', parallel=False)
        return None
...
Traj = Prot_Hop(r"./", cheap=False, xyz_out=False, verbose=True, center=False, store_force_test=False)